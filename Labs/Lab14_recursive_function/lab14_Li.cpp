/******************************************************************************
                                QiuLiangJun Li
                                Lab 14, Recursive 

*******************************************************************************/

#include <iostream>
using namespace std;

void cheers(int n){
    // stop the recursive function
    if(n==1){
        cout<<"STOP"<<endl;
    }
    else{
        cout<<(n*2)<<"\t";
        // recursive function
        cheers(n-1);
    }
}


// recursive function to validate a number
int getnumber(){
    int num;
    cout << "Please enter a positive interger: ";
    cin >> num;
    
    if(num <= 0){
        cout << "ERROR! enter a positive number: ";
        return getnumber();
    }
    return num;
}


// Linear recursive function

int linear_recursive(int m){
    if(m>1){
        return (m+1)*linear_recursive(m-2);
    }
    else{
        return 10;
    }
}

int main()
{   
    cout << "-----Example 3 -----" << endl;
    int x = linear_recursive(7);
    cout << "Final x = "<< x << endl;
    
    /*
    function
    iteration   |   int m   | if(m>1){ return (m+1)*linear_recursive(m-2);   | else {return 10;
        1       |   m = 7   | (7+1=8)*linear_recursive(7-2=5);               | skip
        2       |   m = 5   | (8)*(5+1=6)*linear_recursive(5-2=3);           | skip
        3       |   m = 3   | (8)*(6)(3+1=4)*linear_recursive(3-2=1);        | skip
        4       |   m = 1   | skip                                           | 10
        
        Final x = 8 * 6 * 4 * 10 = 1920
    */
    
    cout << "-----Example 1 -----" << endl;
    cheers(5); // first time calling function
    
    /*
    function
    iteration   |   int n   |   if(n==1)    |   else { cout<<(n*2)<<"\t";   |   cheers(n-1)
        1       |   5       |   false       |   cout --> 5*2=10;            |   cheers(5-1=4)
        2       |   4       |   false       |   cout --> 4*2=8;             |   cheers(4-1=3)
        3       |   3       |   false       |   cout --> 3*2=6;             |   cheers(3-1=2)
        4       |   2       |   false       |   cout --> 2*2=4;             |   cheers(2-1=1)
        5       |   1       |   true (stop the recursive function)
    */
    
    cout << "-----Example 2 -----" << endl;
    getnumber();
    
    
    return 0;
}